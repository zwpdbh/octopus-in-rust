...
- **Training:** REINFORCE with entropy regularization, return normalization, masked softmax, and supervised fine-tuning on the best trajectory.
- **Search:** using the trained Burn network as a policy prior inside UCT/MCTS.
...
1. **[Why RL for build orders?](00-why-rl.md)** — Why build-order optimization is hard, and how a learned direction policy plus MCTS beats hand-written heuristics.
...
7. **[Training with REINFORCE](06-training-pipeline.md)** — Episode rollouts, return normalization, masked log-probabilities, entropy regularization, and cross-entropy fine-tuning on the best trajectory.
8. **[MCTS search with a Burn prior](07-mcts-search.md)** — UCT/PUCT selection, 6-way expansion, direction priors from the Burn network, rollout, and backup.
...
10. **[Benchmarking and tuning](09-benchmarking-and-tuning.md)** — Metrics, diagnosing failure modes, and tuning `c_puct`, iteration budgets, entropy, and reward coefficients.
11. **[The heuristic layer](10-heuristic-layer.md)** — The deterministic rules that turn each of the six network directions into a build/upgrade action.
12. **[Glossary](glossary.md)** — Terms used throughout the tutorial.
13. **[Formal model](model.md)** — Exact definitions, constraints, and assumptions (optional reference).
