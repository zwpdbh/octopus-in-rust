...
All static unit knowledge is accessed through the `BlueprintLibrary` abstraction in `crates/faf-sim/src/units/mod.rs`. `BlueprintLibrary` owns a dedicated Bevy ECS `World` where each unit definition is a blueprint entity with components such as `UnitKindComp`, `FactionComp`, `UnitCostComp`, `BuildPower`, `EconomyProfile`, `StorageProfile`, `BuildRecipeComp`, and `UpgradeRecipesComp`. It is built once from the raw `faf-units` index. The rest of `faf-sim` does not import `faf-units` directly.

```rust
// crates/faf-sim/src/units/mod.rs ~line 37 — BlueprintLibrary (abbreviated)
