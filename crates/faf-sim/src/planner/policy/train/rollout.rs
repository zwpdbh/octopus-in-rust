...
use crate::engine::unit_graph::GoalProject;
...
pub(crate) fn eco_rollout(
    ...
    if !builders.is_empty() {
        state.graph.goal_project = Some(GoalProject {
            goal: *goal,
...
pub(crate) fn rush_rollout(
    ...
    if let Some(ref mut gp) = state.graph.goal_project {
...
fn run_rollout(
    ...
    for _ in 0..steps {
        if stop_on_goal_completion && state.goal_reached_from_project() {
            ...
            result.time_to_finish_secs = Some((state.graph.time - start_time) as f32);
            break;
        }

        let prev_mass_storage = state.engine.economy.mass_storage.value();
        let mass_income = state.engine.economy.net_mass_income.value() * dt;

        state.tick(dt);

        let mass_spent_this_tick =
            (prev_mass_storage + mass_income - state.engine.economy.mass_storage.value()).max(0.0);
    ...
    result.mass_hoarded = state.engine.economy.mass_storage.value()
        > state.engine.economy.mass_storage_cap.value()
            * (config.mass_storage_hoarding_ratio as f64);
...
/// produces.
fn is_energy_stalled(state: &Simulation) -> bool {
    state.engine.economy.energy_storage.value() <= 1e-6
}
...
fn select_builders_by_power(
    ...
    let total_bp = state.graph.total_active_build_power();
    if total_bp <= 0.0 {
        return Vec::new();
    }
    ...
    let mut builders: Vec<_> = state
        ...
        .active_units()
        .into_iter()
        .filter(|&id| builder_power(id, &state.graph.graph, units) > 0.0)
        .collect();

    builders.sort_by(|&a, &b| {
        let rate_a = builder_power(a, &state.graph.graph, units);
        let rate_b = builder_power(b, &state.graph.graph, units);
        rate_b
            .partial_cmp(&rate_a)
    ...
    for id in builders {
        ...
        accumulated += builder_power(id, &state.graph.graph, units);
        selected.push(id);
    }
    selected
}
...
impl RolloutGoalExt for Simulation {
    fn goal_reached_from_project(&self) -> bool {
        self.graph
            .goal_project
            .as_ref()
