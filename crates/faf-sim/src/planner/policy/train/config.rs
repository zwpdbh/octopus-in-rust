...
/// Configuration for the standalone eco-network training run.
#[derive(Debug, Clone, Copy)]
pub struct TrainEcoConfig {
    ...
    /// Target mass income (per second) that ends an eco-only episode.
    pub eco_target_mass_income: f64,
}

impl Default for TrainEcoConfig {
    fn default() -> Self {
        Self {
            episodes: 200,
            max_steps: 500,
            ...
            eco_target_mass_income: 1000.0,
        }
    }
}

/// Configuration for a rush-policy training run.
#[derive(Debug, Clone, Copy)]
