...
pub struct EconomyState {
    /// Stable mass income per second generated by producers (before construction drain).
    pub mass_income: MassRate,
    /// Stable energy income per second generated by producers (before construction drain).
    pub energy_income: EnergyRate,
    /// Net mass income per second after applying construction drain and any scaling.
    #[serde(default)]
    pub net_mass_income: MassRate,
    /// Net energy income per second after applying construction drain.
    #[serde(default)]
...
pub struct GraphTickResult {
    ...
    pub mass_stalled: bool,
    /// Net mass income per second after applying the energy-stall scaling and
    /// subtracting the actual mass drain.
    pub net_mass_income: MassRate,
    /// Net energy income per second after subtracting the actual energy drain.
    pub net_energy_income: EnergyRate,
}
...
pub fn apply_tick_graph(
    ...
    let net_mass_income =
        scaled_net_mass_income - MassRate::from_raw(mass_consumed.value() / dt.value());
    let net_energy_income =
        state.energy_income - EnergyRate::from_raw(energy_consumed.value() / dt.value());
