...
impl BrainError {
    /// Classify an llm-provider provider error into a typed BrainError.
    pub fn from_kosong_error(err: llm_provider::ChatProviderError) -> Self {
        match err.kind {
            llm_provider::ChatProviderErrorKind::Status {
                status_code,
                message,
                ..
            ...
            llm_provider::ChatProviderErrorKind::Connection(message) => {
                BrainError::ApiConnection(message)
            }
            llm_provider::ChatProviderErrorKind::Timeout(message) => {
                BrainError::ApiTimeout(message)
            }
            llm_provider::ChatProviderErrorKind::EmptyResponse => BrainError::ApiEmptyResponse,
            llm_provider::ChatProviderErrorKind::Other(message) => BrainError::Llm(message),
        }
    }
