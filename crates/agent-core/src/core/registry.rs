...
use llm_provider::tooling::{CallableTool, HandleResult, Tool, ToolResult, Toolset};
...
///
/// Implements [`llm_provider::Toolset`] so it can be passed to [`llm_provider::step`].
...
impl ToolRegistry {
    ...
    /// Register a type-safe [`llm_provider::tooling::CallableTool2`] via the adapter.
    pub fn register_typed<T: llm_provider::tooling::CallableTool2 + 'static>(
        &self,
        tool: T,
        source: impl Into<String>,
    ) {
        self.register(
            Box::new(llm_provider::tooling::CallableTool2Adapter::new(tool)),
            source,
        );
    }
