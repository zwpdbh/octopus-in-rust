# Agent-Core Crate Implementation Tracker

Tracks the extraction of a reusable **agent-core** crate from `octopus-cli` and its integration into `qqbot-core`.

- **Plan:** [`docs/plans/14-agent-core-architecture.md`](../../plans/14-agent-core-architecture.md)
...
| 2026-06-15 | Layer `agent-core` into `core` / `tools` / `session` / `hooks`; add `ApprovalPolicy`, `ToolSource`, and streaming API. | Keeps `qqbot-core` minimal while letting `octopus-cli` opt into full features later. |
...
- [x] Create `agent-core` workspace member.
...
- [x] Define agent-loop types and logic:
  ...
  - [x] LLM-provider resolution via `llm-provider`.
...
- [x] Implement plugin discovery via `agent_core::tools::plugin::ExtismPluginSource`.
...
- The agent-core crate is a cross-cutting architectural change, not a direct Python→Rust port.
