# llm-provider Mirror Comparison: Python ↔ Rust

> **Scope:** Compare the Python `kosong` abstraction layer (`tmp/kimi-cli/packages/kosong/`) with the Rust `llm-provider` crate (`llm-provider/`). The Rust crate is a faithful rewrite of the Python package, consumed by `octopus-cli` as a local path dependency.
>
> **Legend:**
...
| Metric | Python kosong | Rust llm-provider | Notes |
|--------|--------------|-------------|-------|
...
|--------|------|--------|-------|
| `kosong.step()` | `llm_provider::step()` | ✅ | Same signature; Rust uses `tokio::task::JoinHandle` instead of `asyncio.Future` |
| `kosong.generate()` | `llm_provider::generate()` | ✅ | Same streaming semantics |
...
🚫 **`kimi-cli` does not use `SimpleToolset`.** It has its own `KimiToolset` with dedup, hooks, telemetry, and MCP support. `SimpleToolset` is still implemented for other `llm-provider` consumers.
...
| Error conversion | `convert_reqwest_error()` | ✅ | Maps to llm-provider error types |
...
🚫 **`kimi-cli` does not use Echo/Mock providers.** They are testing utilities for the `llm-provider` crate itself.
...
🚫 **`kimi-cli` does not use Contrib Context.** It has its own session/history management. This module is for standalone `llm-provider` consumers.
...
| MCP tool wrapper | `tooling/mcp.py` | ❌ Not started | Medium | Wraps MCP client as `llm-provider` `Tool`. 🚫 `kimi-cli` has its own MCP layer in `KimiToolset`. |
...
🚫 **`kimi-cli` does not use `EmptyToolset` or the llm-provider MCP wrapper.** It has its own `KimiToolset` with integrated MCP support. These would only be needed for other `llm-provider` consumers.
...
| Transport | `httpx.HTTPError` → kosong errors | `reqwest::Error` → llm-provider errors |
| Empty response | `APIEmptyResponseError("msg")` | `ChatProviderError::new("msg")` with matching string |

The Rust `llm.rs` in `octopus-cli` classifies llm-provider error strings back into specific `OctopusError` variants.
...
├── `LLM::complete()` → calls `llm_provider::generate()` via `llm-provider` crate
├── `LLM::generate_streaming()` → calls `llm_provider::generate()` with `on_message_part` + `on_tool_call` callbacks
├── `build_llm_provider()` → builds `llm_provider::provider::kimi::Kimi`, `OpenAILegacy`, or `OpenAIResponses`
└── Type conversions: `wire_to_llm_*` / `llm_to_wire_*`

octopus-cli/src/soul/mod.rs (KimiSoul)
├── `_run_step_once_inner()` → manually implements step logic (NOT using llm_provider::step)
│   └── Calls `LLM::generate_streaming()` (streams message parts via llm_provider::generate)
...
**Important:** `KimiSoul` now calls `llm_provider::step_with_callbacks()` directly. It no longer uses the manual `LLM::generate_streaming()` + early tool dispatch pipeline. This means:

- ✅ `KimiSoul` streams message parts to the user in real-time
- ✅ `llm_provider::step_with_callbacks` dispatches tools **during streaming** as they arrive from the stream
- ✅ `llm_provider::step_with_callbacks` executes tools concurrently (not sequentially)
- ✅ `KimiSoul` handles D-Mail, hooks, telemetry, MCP, etc. at a higher layer
- ✅ `KimiToolsetHandle` implements `llm_provider::Toolset` for `Arc<KimiToolset>`

The llm-provider crate is complete and functional; `octopus-cli` uses `llm_provider::step_with_callbacks()` with the `on_tool_result` callback for eager wire events.
...
1. ~~**Wire `llm_provider::step()` into `KimiSoul`**~~ ✅ **DONE** — `kimisoul.rs` now calls `llm_provider::step_with_callbacks()` directly. `KimiToolsetHandle` implements `llm_provider::Toolset` for `Arc<KimiToolset>`.

### P1 — Important
2. ~~**Add `on_tool_result` to Rust `llm_provider::step()`**~~ ✅ **DONE** — `StepResult::tool_results_with_callback()` and `step_with_tool_result_callback()` implemented. Fires callback synchronously as each `JoinHandle` resolves.
3. ~~**Implement OpenAI Responses provider**~~ ✅ **DONE** — `llm_provider::provider::openai_responses::OpenAIResponses` implemented with streaming and non-streaming support.
...
7. **MCP tool wrapper** — Bridge MCP tools into `llm-provider` `Toolset`. `octopus-cli` already has its own MCP layer; this would make MCP available to other llm-provider consumers. 🚫 Not used by `kimi-cli`.
