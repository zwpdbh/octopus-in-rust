...
pub fn register_tools(_input: String) -> FnResult<String> {
    let tools = vec![ToolDef {
        name: "faf_party_parse_message".to_string(),
        description: "Parse a user's message to detect FAF party intent (join/leave/unknown) and availability time window.".to_string(),
        prompt_fragment: Some(
            "When a user says they can play FAF, want to join a party, or want to leave/cancel, call faf_party_parse_message to extract intent and availability.".to_string(),
        ),
...
pub fn execute(input: String) -> FnResult<String> {
    if input.is_empty() {
        return Ok(serde_json::to_string(&serde_json::json!({"error":"empty input"}))?);
    }
    ...
    let result = match parsed.tool.as_str() {
        "faf_party_parse_message" => parse_message_tool(parsed.arguments),
...
fn parse_message_tool(args: serde_json::Value) -> serde_json::Value {
    let args: ParseMessageArgs = match serde_json::from_value(args) {
        Ok(a) => a,
    ...
    let (intent, availability) = parse_message(&args.message, now);
...
fn parse_relative_minutes(text: &str) -> i64 {
    ...
    // Specific Chinese word forms.
    if Regex::new(r"半\s*个?\s*小时\s*后").unwrap().is_match(&normalized) {
        return 30;
    }
    if Regex::new(r"一\s*个?\s*小时\s*后").unwrap().is_match(&normalized) {
        return 60;
    }
    if Regex::new(r"两\s*个?\s*小时\s*后|二\s*个?\s*小时\s*后").unwrap().is_match(&normalized) {
        return 120;
    }
    if Regex::new(r"三\s*个?\s*小时\s*后").unwrap().is_match(&normalized) {
        return 180;
    }
    ...
    // Numeric hours, including "x个半小时后".
...
fn parse_message(message: &str, now: chrono::DateTime<chrono::FixedOffset>) -> (Intent, Option<Availability>) {
    let normalized = message.to_lowercase();

    // Leave detection.
    let leave_signals = ["不玩了", "cancel", "退出", "leave", "/faf leave", "不打了", "不来", "算了"];
    ...
    match parse_availability(message, now) {
        Some(avail) => (Intent::Join, Some(avail)),
        None => (Intent::Unknown, None),
    }
}

/// Parse a Chinese availability expression into a start/end window.
/// Default end is 22:00. Past times roll forward to tomorrow.
fn parse_availability(message: &str, now: chrono::DateTime<chrono::FixedOffset>) -> Option<Availability> {
    ...
    // Helper: build datetime from date + time, rolling to tomorrow if past.
    let at_time = |date: chrono::NaiveDate, time: NaiveTime| {
        let dt = now.timezone().from_local_datetime(&date.and_time(time)).single()?;
        if dt <= now {
            now.timezone().from_local_datetime(&tomorrow.and_time(time)).single()
        } else {
            Some(dt)
        }
    };
    ...
    if let Some(caps) = range_re.captures(&normalized) {
        ...
        let start_minute: i32 = caps.get(2).and_then(|m| m.as_str().parse().ok()).unwrap_or(0);
        let end_hour: i32 = caps.get(3)?.as_str().parse().ok()?;
        let end_minute: i32 = caps.get(4).and_then(|m| m.as_str().parse().ok()).unwrap_or(0);
        ...
        if end <= start {
            end = now.timezone().from_local_datetime(&tomorrow.and_time(end_time)).single()?;
        }
        return Some(Availability {
            start: start.to_rfc3339(),
            end: end.to_rfc3339(),
            description: format!("{}:{}到{}:{}", start_hour, start_minute, end_hour, end_minute),
        });
    }

    // Relative time expressions: 半小时后, 30分钟后, 一小时后, etc.
    ...
    if normalized.contains("明天晚上") {
        let start = now.timezone().from_local_datetime(&tomorrow.and_time(evening_start)).single()?;
        let end = now.timezone().from_local_datetime(&tomorrow.and_time(default_end_time)).single()?;
        return Some(Availability {
            start: start.to_rfc3339(),
            end: end.to_rfc3339(),
    ...
    let hour_re = Regex::new(r"(?:晚上|傍晚|下午)?\s*(\d{1,2})\s*点\s*(?:(\d{1,2})\s*分?)?").unwrap();
    if let Some(caps) = hour_re.captures(&normalized) {
        let mut hour: i32 = caps.get(1)?.as_str().parse().ok()?;
        // Heuristic: "晚上8点" -> 20:00.
